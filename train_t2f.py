...
# from fid import calc # NOT Used
from models.maskdit import Precond_models
from train_utils.loss import Losses
from train_utils.datasets import Text2FaceDataset
...
def train_loop(args):
    ...
    dataset = Text2FaceDataset(config.data.root, config.data.text_npz, transform=transform, resolution=config.data.resolution)
    ...
    running_loss = 0
    start_time = time()
    label_table = torch.eye(config.model.num_classes, device=device) # consider removing this
